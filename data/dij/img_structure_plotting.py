    ...
    if isinstance(zslices,int):
        ...
        for z in zslices:
            ...
            if plotstuff:
                ...
                plt.show()
    if printSliceCount:
        print('Structure(s)' + str(count) + ' is in ' + str(num) + ' of ' + str(slices) + ' plotted slices')
...
print(np.unique(struct_img_arr))
...
#plt.show()
flatNP = sub_sampled_img_struct.flatten(order='F')
countflatNP = np.array([i if 0.1 < flatNP[i] else 0 for i in range(len(flatNP)) ])

lp = np.in1d(countflatNP, voxels)
print(np.count_nonzero(lp), np.count_nonzero(countflatNP), 'voxels taken into account and total voxels')
plt.imshow(sub_sampled_img_struct[:,:,80],cmap='autumn',interpolation='none')
plt.show()
